...
def run_training(
    name, team, name_of_agent, max_eps, write_to_file=False, agent_class="AgentPPOSEQ"
):

    agent_folder = Path(os.path.join(model_folder, agent_class, name_of_agent))
    ...
    for i in range(max_eps):
        ...
        for j in range(500):
            actions_info = {
                agent_name: agent.get_action_with_prob(observations[agent_name])
...
if __name__ == "__main__":
    ...
    run_evaluation(name, team, technique, 100, agents, wrap, write_to_file=False)
