...
def run_training(name, team, name_of_agent, max_eps, write_to_file=False):
    ...
    if technique == "DDQN":
        agents = {
            f"blue_agent_{agent}": AgentDDQN(
                wrapped_cyborg,
                observation,
                wrapped_cyborg.action_space(f"blue_agent_{agent}"),
                f"blue_agent_{agent}",
                device=device,
            )
            ...
            for j in range(500):
                ...
                for agent_name in agents:
                    agent_state = (
                        ...
                        if agent_name in observations.keys()
                        else None
                    )
                    agent_action = (
                        actions[agent_name]
                        if agent_name in actions.keys()
                        else action_spaces[agent_name].sample()
                    )
                    agent_reward = rew[agent_name] if agent_name in rew.keys() else 0
                    agent_done = done[agent_name] if agent_name in done.keys() else True
        ...
        for i in range(max_eps):
            ...
            for j in range(100):
                timestep += 1
                actions = {
                    agent_name: agent.get_action(observation[agent_name], agent.memory)
                    for agent_name, agent in agents.items()
                    if agent_name in wrapped_cyborg.agents
                }
                    ...
                    o.append(
                        {
                            ...
                            for agent_name in observation.keys()
                        }
                    )
                r.append(mean(rew.values()))
